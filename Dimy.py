...
# Diffie Hellman method generates private and public keys and returns them
# - The minimum size of the key supported is 512 bits
# - The public key is also called EphID
def GenerateKeys():
...
def createQBF(dbf_list, positive):
    ...
    if positive:
        print("Task 9  | Combined DBFs into CBF (index with '1'):")
        print(getSetBits(qbf.bit_array))
        print("Task 9  | Sending CBF to server")
    else:
        print("Task 8  | Combined DBFs into QBF (index with '1'):")
        print(getSetBits(qbf.bit_array))
    ...
    while threadAlive:
        ...
        if time.time() - T_last_broadcast >= 3: # every 3 second, broadcast a EphID_shares
            ...
            if x >= 0.5: # only when random >= 0.5 transfer the shares[i]
                msg = (str_EphID_sha1, EphID_shares[i])
                i += 1
            ...
            else: # if random < 0.5, we skip share[i]
                ...
                msg0 = (0, 0) # When random < 0.5, shares[i] is not broadcast, instead send msg0(0,0). This msg0 broadcast maybe delete with using select.select()
        ...
        # import msvcrt is used for Windows operation system. For Linux, it is not useful.
        ...
        # receive msg from other clients
            if datasource == UDPreceivesocket:
                ...
                # When datasource = serverTCPsocket from the server
                msgFromServer = datasource.recv(1024).decode('utf-8')
                print(msgFromServer)
             
        #--------------------This part is handling the UDP client EncID-----------------
        if addr_client[1] != broadcastsocket.getsockname()[1] and data_client != (0,0): # ingore the broadcast message from itself and ingore the broadcast of the dropping mechanism.
            if data_client[0] not in dict_received_EphID:
                ...
                print("Task 3-B|      Received share: "+str(data_client[1])[1:6]+"...")
                dict_received_EphID[data_client[0]] = [[data_client[1]], time.time()]
                print("Task 3-C|      Collected "+str(len(dict_received_EphID[data_client[0]][0]))+" shares (hash "+str(data_client[0])[:7]+")")
            ...
            if len(dict_received_EphID[data_client[0]][0]) == 3:
                ...
                if str_reconst_secret_sha1 == data_client[0]: # reconstructed secret = hashed secret
                    ...
                    if EncID not in list_EncID:
                        ...
                        print("Task 7-A| State of DBF "+str(dbf_count)+" (index with '1'):")
